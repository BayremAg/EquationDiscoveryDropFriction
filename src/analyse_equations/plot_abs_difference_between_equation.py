...
def abs_difference_between_equation(args,proposed_equations, df, all_data_dfs, index_0, index_1):
    prefix_0 = df.loc[index_0].loc['equation']
    prefix_1 = df.loc[index_1].loc['equation']
    tree_0 = map_equation_to_syntax_tree(args, df.loc[index_0].loc['equation'], infix=False, catch_exceptions=False)
    tree_0.constants_in_tree = proposed_equations[prefix_0]['all_data']['train']['constants']
    tree_1 = map_equation_to_syntax_tree(args, df.loc[index_1].loc['equation'], infix=False, catch_exceptions=False)
    tree_1.constants_in_tree = proposed_equations[prefix_1]['all_data']['train']['constants']
    system_id_column = args.system_id_column
    system_ids = all_data_dfs[system_id_column].unique()
    id_list = []
    mean_abs_error = []
    for id in system_ids:
        df_system = all_data_dfs[all_data_dfs[system_id_column] == id]
    ...
    # ax2.set_ylabel('MSE')
    fig.tight_layout()
    plt.savefig(args.save_path / "difference_between_equation.pdf")
    plt.show()
