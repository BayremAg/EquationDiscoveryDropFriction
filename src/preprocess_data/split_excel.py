...
def replace_excel_name(row):
    if row['excel_name'] in excel_name_map:
        return excel_name_map[row['excel_name']]
    else:
        return row['excel_name']

def split_dataset(args):
    df = pd.read_excel(args.path_to_excel, index_col='sequence')
    df = df.drop(columns=['Unnamed: 0'])
    df['excel_name'] = df.apply(lambda row: f"vis={float(row['viscosity(mPa.s)']):.2f}, "
                                            f"fric={float(row['friction_coef']):.2f}, "
                                            f"adv={float(row['static_adv(degree)']):.2f}, "
                                            f"rec={float(row['static_rec(degree)']):.2f}", axis=1)
    df['excel_name'] = df.apply(replace_excel_name, axis=1)
    print(f"Unique values: {df['excel_name'].unique()}")
...
if __name__ == '__main__':
    ...
    args.output_folder = ROOT_DIR / 'data/Aug_2025'
    # Load the CSV file
    split_dataset(args)
